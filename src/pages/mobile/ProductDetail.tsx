  ...
  return (
    <div className="flex flex-col min-h-full bg-white pb-40">
      ...
      <div className="p-6">
        ...
        <p className="text-gold font-bold text-xl mb-6">₹{product.price} <span className="text-sm font-normal text-gray-500">/ kg</span></p>

        <p className="text-gray-500 text-sm leading-relaxed mb-8">
          Authentic, rich, and made with pure desi ghee. A signature delicacy from Shri Badrinarayan Papriwale, perfect for every celebration.
        </p>

        {/* Size Selection */}
        <div className="mb-8">
          <h3 className="text-sm font-bold text-gray-800 mb-3 uppercase tracking-wider">Select Quantity</h3>
          <div className="flex gap-2">
            {["250g", "500g", "1kg", "2kg"].map(size => (
              <button 
                key={size}
                onClick={() => setSelectedSize(size)}
                className={`flex-1 py-2 text-sm font-semibold rounded-full border transition-colors ${
                  selectedSize === size ? "bg-maroon text-white border-maroon" : "bg-white text-gray-600 border-gray-200"
                }`}
              >
                {size}
              </button>
      ...
      <div className="fixed bottom-[64px] left-0 right-0 max-w-md mx-auto p-4 bg-white border-t border-gray-100 pb-safe shadow-[0_-5px_20px_rgba(0,0,0,0.05)] z-30">
        <button onClick={() => { addToCart(product, selectedSize, qty); navigate("/cart"); }} className="w-full bg-maroon text-cream font-bold py-4 rounded-xl hover:bg-maroon-light transition-colors shadow-md flex items-center justify-center gap-2 text-lg">
